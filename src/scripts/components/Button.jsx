...
export default function Button({ variant = 'primary', children = 'Button text', size = '', icon = true, dark = false }) {
  const elProps = {
    href: '#',
    className: classnames(['btn', variant ? `btn--${variant}` : false, size ? `btn--${size}` : false, dark ? 'btn--dark' : false]),
  };
